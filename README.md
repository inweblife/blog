...
- `*.html` в основната директория - канонични страници и публикации
- `assets/` - CSS и JavaScript
- `image/` - изображения
- `feed.xml` - RSS feed
- `sitemap.xml` - карта на сайта
- `vercel.json` - clean URL настройки и legacy redirects от стари `/posts/...` адреси
...
- За нови публикации копирай структурата от съществуващите root HTML файлове
- Обновявай `feed.xml` и `sitemap.xml` при нови публикации
- Не създавай копия в `posts/`; старите `/posts/...` адреси трябва да останат само като redirects към каноничните URL-и
